...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' scrolltab.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" scrolltab.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' scrolltab.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" scrolltab.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' scrolltab.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" scrolltab.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' scrolltab.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' scrolltab.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" scrolltab.podspec
...
pod repo push ${Repo} scrolltab.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
